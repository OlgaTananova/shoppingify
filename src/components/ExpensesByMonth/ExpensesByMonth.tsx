...
export default function ExpensesByMonth() {
  ...
  //           date: monthAndYear(),
  //           categories:
  //             value?.items?.reduce((p, v) => {
  //               if (!p[v!.categoryId]) {
  //                 p[v!.categoryId] = {
  //                   categoryName:
  //                     categories.find((c) => c._id === v?.categoryId)
  //                       ?.category || "Unknown category",
  //                   total: v?.price || 0,
  //                   items: {
  //                     [v!.itemId]: {
  //                       itemName:
  //                         items.find((i) => i._id === v?.itemId)?.name ||
  //                         "Unknown item",
  //                       total: v?.price || 0,
  //                     },
  //                   },
  ...
  //               categoryName:
  //                 categories.find((c) => c._id === v?.categoryId)?.category ||
  //                 "Unknown category",
  //               total: v?.price || 0,
  //               items: {
  //                 [v!.itemId]: {
  ...
  //             };
  //           } else {
  //             prev[monthAndYear()].categories[v!.categoryId].total +=
  //               v?.price || 0;
  //             // @ts-ignore
  //             if (
  ...
  //             ) {
  //               // @ts-ignore
  //               prev[monthAndYear()].categories[v!.categoryId].items[
  //                 v!.itemId
  //               ] = {
  //                 itemName:
  //                   items.find((i) => i._id === v?.itemId)?.name ||
  ...
  // const sortedSpendingByMonth = useMemo(
  //   () =>
  //     Object.entries(spendingByMonth).sort((a, b) => {
  ...
  //       <select
  //         className="statistics__byMonth-month-selector"
  ...
  //         Object.entries(spendingByMonth[selectedMonth].categories).map(
  //           (value) => (
  //             <CategoriesByMonth
  //               category={value[1] as IExpensesBySingleCategory}
  //               key={value[0]}
  //               index={value[0]}
  //               totalInMonth={spendingByMonth[selectedMonth].total}
  //             />
  //           ),
  //         )
  //       : null}
  ...
  //       <p className="statistics__ByMonth-salesTax-value">{`$${
  //         spendingByMonth[selectedMonth]?.salesTax.toFixed(2) || 0
  //       }`}</p>
  //     </div>
  //     <div className="statistics__ByMonth-total-section">
  //       <p className="statistics__ByMonth-total-heading">Total</p>
  //       <p className="statistics__ByMonth-total-value">{`$${
  //         spendingByMonth[selectedMonth]?.total.toFixed(2) || 0
  //       }`}</p>
  //     </div>
  //   </div>
  // );
