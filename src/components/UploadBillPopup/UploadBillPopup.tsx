...
function UploadBillPopup() {
  ...
  // );
  // const [itemsFromSLWithNames, setItemsFromSLWithNames] = useState<
  //   IFullShoppingItem[]
  // >([]);
  ...
  // });
  // const [errorsForUploadedItems, setErrorsForUploadedItems] = useState<
  //   IUploadedShoppingItemErrors[]
  // >([]);
  ...
  //         'sales-tax': { value: Number(item.salesTax) || 0, required: true },
  //       });
  //       return;
  //     }
  //     if (
  //       editUploadedItemsForm.values.find(
  //         (i) => i.itemName.value === item?.itemName,
  //       )
  //     )
  //       return;
  ...
  //     ]);
  //   });
  //   const taxAndDateObject = uploadedItems?.find(
  //     (item) => item && Object.keys(item).includes('dateOfPurchase'),
  //   );
  //   setDateOfBill(
  //     () => taxAndDateObject?.dateOfPurchase || new Date().toISOString(),
  //   );
  // }, [uploadedItems]);
  //
  ...
  // useEffect(() => {
  //   const items =
  //     uploadedItems?.reduce((acc: IShoppingItem[], item) => {
  ...
  // useEffect(() => {
  //   const items =
  //     uploadedItems?.reduce((prev: IFullShoppingItem[], item) => {
  ...
  //   if (!isActiveSL) {
  //     return uploadedBill &&
  //       uploadedBill?.find((i) => i?.itemName === item?.itemName?.value)?.itemId
  //       ? ''
  //       : 'upload-bill-popup__list-item_notfound';
  //   }
  //   return mergedSL &&
  //     mergedSL?.find((i) => i?.itemName === item?.itemName?.value)?.itemId
  //     ? ''
  //     : 'upload-bill-popup__list-item_notfound';
  // };
  //
