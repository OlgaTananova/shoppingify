...
function UploadBillPopup() {
  ...
  const handleUploadBillClickButton = () => {
    if (activeShoppingList) {
      ...
      dispatch(setIsLoadingTrue());
      dispatch(mergeBill({ items: uploadedBill, salesTax: editSalesTaxForm.values['sales-tax'].value, date: dateOfBill })).unwrap()
        .then((data) => {
        })
        .catch((err) => {
          dispatch(setShowErrorTrue(err.message));
  ...
  const highLightItem = (item: IUploadedShoppingItemInitialState) => {
    if (!isActiveSL) {
      return uploadedBill && uploadedBill?.find((i) => i?.itemName === item?.itemName?.value)?.itemId ? '' : 'upload-bill-popup__list-item_notfound';
    }
  ...
  return (
    <div className={`upload-bill-popup ${!showUploadBillPopup && 'upload-bill-popup_inactive'}`}>
      <div className="upload-bill-popup__inner-container">
        ...
        <span className="upload-bill-popup__warning">Items with this color are neither in the shopping list nor in the list of the items. </span>
        ...
        <div className="upload-bill-popup__actions-section">
          ...
          <button
            disabled={uploadedItems?.length === 0 || !isUploadedItemsValid || !isActiveSL}
            type="button"
            onClick={handleMergeListPopupMergeBtnClick}
            className={`${(!isUploadedItemsValid || uploadedItems?.length === 0 || !isActiveSL) && 'upload-bill-popup__action-btn_disabled'} upload-bill-popup__action-btn upload-bill-popup__action-btn_merge`}
          >
            Merge
            List
          </button>
          ...
          <button
            ...
            className={`${(!isUploadedItemsValid || uploadedItems?.length === 0 || isActiveSL) && 'upload-bill-popup__action-btn_disabled'} upload-bill-popup__action-btn upload-bill-popup__action-btn_upload-list`}
          >
            Upload
            list
          </button>
