...
import {
  ...
  IItem, IShoppingItem, ISortedItems,
} from '../../types';
...
function Statistics() {
  ...
  const topCategories = useMemo(() => shoppingItems.reduce((prev, curr) => {
  ...
  const topSortedItems = useMemo(() => Object.entries(topItems).sort((a, b) => b[1].price - a[1].price).slice(0, 5), [topItems]);
  // list of top categories by value to display in the top categories section
  const topSortedCategories = useMemo(() => Object.entries(topCategories).sort((a, b) => b[1].price - a[1].price), [topCategories]);
