...
export default function ExpensesByYear() {
    ...
    const sortedExpensesByYear = shoppingLists.reduce((prev, value) => {
      ...
      if (!prev[year]) {
        prev[year] = {
          date: year,
          categories:
            value?.items?.reduce((pr, vl) => {
              if (!pr[vl!.categoryId]) {
                pr[vl!.categoryId] = {
                  categoryName:
                    categories.find((c) => c._id === vl?.categoryId)
                      ?.category || 'Unknown category',
                  total: vl?.price || 0,
                  items: {
                    [vl!.itemId]: {
                      itemName:
                        items.find((i) => i._id === vl?.itemId)?.name ||
                        'Unknown item',
                      total: vl?.price || 0,
                    },
                  },
        ...
        value?.items?.forEach((v) => {
          if (!prev[year].categories[v!.categoryId]) {
            ...
            if (!prev[year].categories[v!.categoryId].items[v!.itemId]) {
              ...
              // @ts-ignore
              prev[year].categories[v!.categoryId].items[v!.itemId].total +=
                v?.price || 0;
            }
          }
        });
  ...
  const sortedSpendingByYear = useMemo(
    () =>
      Object.entries(spendingByYear).sort(
  ...
  return (
    <div className="statistics__byYear">
      ...
      {Object.keys(spendingByYear).length !== 0 ? (
        <select
          className="statistics__byYear-month-selector"
      ...
      {Object.keys(spendingByYear).length !== 0
        ? spendingByYear[selectedYear]?.categories &&
          Object.entries(spendingByYear[selectedYear].categories).map(
            (value) => (
              <CategoriesByMonth
                category={value[1] as IExpensesBySingleCategory}
                key={value[0]}
                index={value[0]}
                totalInMonth={spendingByYear[selectedYear].total}
              />
            ),
          )
        : null}
      <div className="statistics__byYear-salesTax-section">
        ...
        <p className="statistics__byYear-salesTax-value">{`$${
          spendingByYear[selectedYear]?.salesTax.toFixed(2) || 0
        }`}</p>
      </div>
      <div className="statistics__byYear-total-section">
        <p className="statistics__byYear-total-heading">Total</p>
        <p className="statistics__byYear-total-value">{`$${
          spendingByYear[selectedYear]?.total.toFixed(2) || 0
        }`}</p>
      </div>
    </div>
  );
