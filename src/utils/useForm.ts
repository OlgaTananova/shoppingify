...
function useForm<T>(initialValues: T) {
  ...
  const initialValuesForErrors = Object.fromEntries(
    Object.keys(initialValues).map((key) => [key, '']),
  );
  const [errors, setErrors] = useState(initialValuesForErrors);
  const isDirtyInput = Object.values(initialValues).some(
    (entry) => entry.value === '' && entry.required === true,
  );
  ...
  useEffect(() => {
    const areErrorsEmpty =
      Object.values(errors).every((error) => error === '') || errors === {};
