...
function useForm<T>(initialValues: T) {
    ...
    const initialValuesForErrors = Object.fromEntries(Object.keys(initialValues).map((key)=>{
        return [key, '']
    }));
    const [errors, setErrors] = useState(initialValuesForErrors);
    const isDirtyInput = Object.values(initialValues).some((entry) => {
        return entry.value === '' && entry.required === true;
    });
    ...
    useEffect(() => {
        ...
        const isAnyFieldInvalid = Object.values(values).some(entry => {
            return entry.value === '' && entry.required === true;
        }) || values === {};
